...
export class Lobby {
    ...
    private async endGameUpdateStatsAsync(game: Game): Promise<void> {
        try {
            ...
            if (game.state.winnerNames.length === 0 || !game.finishedAt || this.game.roundNumber <= 1) {
                return;
            }
            ...
            let winner, loser;
            if (game.state.winnerNames.includes(player1.name)) {
                winner = player1;
                loser = player2;
            } else if (game.state.winnerNames.includes(player2.name)) {
                winner = player2;
                loser = player1;
            }

            // If we have a draw (or couldn't determine winner/loser), set as draw
            const isDraw = !winner || !loser || game.state.winnerNames.length > 1;
    ...
    public sendGameState(game: Game, forceSend = false): void {
        ...
        if (game.state.winnerNames.length > 0 && game.finishedAt && !game.statsUpdated) {
