        ...
        this.setEventAbility({
            ...
            immediateEffect: AbilityHelper.immediateEffects.conditional({
                ...
                onTrue: AbilityHelper.immediateEffects.selectCard({
                    ...
                    cardCondition: (card, context) => (!card.hasSomeTrait(Trait.Vehicle)) && (Array.from(context.source.controller.getUnitsInPlay()[0].traits).some((trait) => card.hasSomeTrait(trait))),
                    innerSystem: AbilityHelper.immediateEffects.playCardFromHand({ adjustCost: { costAdjustType: CostAdjustType.Decrease, amount: 5 } })
                }),
            })
        });
    }
