...
export default class ReyKeepingThePast extends NonLeaderUnitCard {
    ...
    public override setupCardAbilities(registrar: INonLeaderUnitAbilityRegistrar, AbilityHelper: IAbilityHelper) {
        registrar.addIgnoreSpecificAspectPenaltyAbility({
            title: TextHelper.performReplacements('While playing this unit, ignore her Heroism aspect penalty if you control Kylo Ren'),
            ignoredAspect: Aspect.Heroism,
            condition: (context) => context.player.controlsLeaderUnitOrUpgradeWithTitle('Kylo Ren')
        });

        registrar.addOnAttackAbility({
            title: TextHelper.performReplacements('You may heal 2 damage from a unit. If it\'s a non-Heroism unit, give a Shield token to it'),
            optional: true,
            targetResolver: {
