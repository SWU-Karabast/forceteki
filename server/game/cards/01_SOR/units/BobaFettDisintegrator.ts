        ...
        this.addOnAttackAbility({
            ...
            immediateEffect: AbilityHelper.immediateEffects.conditional((attackContext) => ({
                ...
                condition: () => this.cardsEnteredPlayThisPhaseWatcher.getCardsPlayed((playedCardEntry) =>
                    playedCardEntry.playedBy === attackContext.source.activeAttack?.getSingleTarget().owner && attackContext.source.activeAttack?.getSingleTarget() === playedCardEntry.card).length === 0 &&
                    attackContext.source.activeAttack?.getSingleTarget().isUnit() && (attackContext.source.activeAttack?.getSingleTarget() as IUnitCard).exhausted,
