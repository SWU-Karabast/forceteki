...
export default class ShuttleST149UnderKrennicsAuthority extends NonLeaderUnitCard {
    ...
    public override setupCardAbilities() {
        this.addTriggeredAbility({
            title: 'You may take control of a token upgrade on a unit and attach it to a different eligible unit.',
            optional: true,
            when: {
                onCardPlayed: (event, context) => event.card === context.source,
                onCardDefeated: (event, context) => event.attack.attacker === context.source,
            },
            targetResolvers: {
                chooseUpgrade: {
