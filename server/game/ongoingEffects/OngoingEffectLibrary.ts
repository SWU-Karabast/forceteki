...
export = {
    ...
    cloneUnit: (target: Card) => OngoingEffectBuilder.card.static(EffectName.CloneUnit, (game) => createGameObject(CloneUnitEffect, game, target)),
    isLeader: () => OngoingEffectBuilder.card.static(EffectName.IsLeader),
    gainAbility: (properties: IAbilityPropsWithType) =>
        OngoingEffectBuilder.card.static(EffectName.GainAbility, (game) => createGameObject(GainAbility, game, properties)),
        ...
        switch (typeof keywordOrKeywordProperties) {
            case 'function':
                return OngoingEffectBuilder.card.dynamic(EffectName.GainKeyword,
                    (target, context, game) => createGameObject(GainKeyword, game, keywordOrKeywordProperties(target, context, game)));
            default:
                return OngoingEffectBuilder.card.static(EffectName.GainKeyword, (game) => createGameObject(GainKeyword, game, keywordOrKeywordProperties));
        }
    },
    gainKeywords: (calculate: (target: any, context: AbilityContext) => KeywordNameOrProperties[]) =>
        OngoingEffectBuilder.card.dynamic(EffectName.GainKeyword, (target, context, game) => createGameObject(GainKeyword, game, calculate(target, context))),
        ...
        switch (typeof modifier) {
            ...
            default:
                return OngoingEffectBuilder.card.static(
                    ...
                    (game) => createGameObject(OngoingEffectValueWrapper, game, modifier, { format: 'give {0}', args: [StatsModifierWrapper.statsModifierDescription(modifier)] })
                );
        }
    },
    ...
    // // Player effects
    additionalAction: () => OngoingEffectBuilder.player.static(EffectName.AdditionalAction, (game) => createGameObject(OngoingEffectValueWrapper, game, true, 'give an additional action')),
    additionalPhase: (properties: { phase: PhaseName }) =>
        OngoingEffectBuilder.player.static(EffectName.AdditionalPhase, (game) => createGameObject(AdditionalPhaseEffect, game, properties.phase)),
