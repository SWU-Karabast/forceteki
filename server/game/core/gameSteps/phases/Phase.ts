...
export abstract class Phase extends BaseStepWithPipeline {
    ...
    protected createPhase(): void {
        this.game.createEventAndOpenWindow(EventName.OnPhaseCreated, null, { phase: this.name }, { triggerMode: TriggerHandlingMode.CannotHaveTriggers }, () => {
    ...
    protected startPhase(): void {
        this.game.createEventAndOpenWindow(EventName.OnPhaseStarted, null, { phase: this.name }, { triggerMode: TriggerHandlingMode.ResolvesTriggers }, () => {
    ...
    protected endPhase(skipEventWindow = false): void {
        if (!skipEventWindow) {
            this.game.createEventAndOpenWindow(EventName.OnPhaseEnded, null, { phase: this.name }, { triggerMode: TriggerHandlingMode.ResolvesTriggers }, () => this.game.currentPhase = null);
