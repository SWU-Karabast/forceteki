...
class OldCard extends OngoingEffectSource {
    ...
    protected upgrades = [] as OldCard[];
    protected childCards = [] as OldCard[];
    ...
    // TODO: when we refactor OldCard we can just make this abstract
    /**
    ...
    /**
     * Equivalent to {@link OldCard.title}
     */
    ...
    //     if (this.isFacedown()) {
    //         return [
    //             { command: 'click', text: 'Select OldCard' },
    //             { command: 'reveal', text: 'Reveal' }
    //         ];
    //     }

    //     const menu = [{ command: 'click', text: 'Select OldCard' }];
    ...
    public addDamage(amount: number) {
        ...
        //     if (this === this.owner.base as OldCard) {
    ...
    // createSnapshot() {
    //     const clone = new OldCard(this.owner, this.cardData);
