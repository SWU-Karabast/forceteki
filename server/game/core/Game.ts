...
import { SwuPgnWriter } from './chat/SwuPgnWriter';
import type {
    DeckRecord,
    ReducedState,
    Seat,
...
export class Game extends EventEmitter {
    ...
    private swuPgnCardId(uuid: string): string {
        const cached = this._swuPgnCardIdByUuid.get(uuid);
        if (cached != null) {
            return cached;
        }
        ...
        let card: any;
        try {
            card = this.getFromUuidUnsafe(uuid as any) ?? null;
        } catch {
            card = null;
        }
        if (!card) {
        ...
        const copyNumber = (this._swuPgnCopyCountByBase.get(base) ?? 0) + 1;
        this._swuPgnCopyCountByBase.set(base, copyNumber);
        const id = copyNumber === 1 ? base : `${base}:${copyNumber}`;
        this._swuPgnCardIdByUuid.set(uuid, id);
        return id;
    }
    ...
    private registerSwuPgnSetupCapture(): void {
        this.on(EventName.OnDeckShuffled, (event: any) => {
            try {
                ...
                if (!player || this._swuPgnDeckOrderCaptured.has(player.id)) {
                    return;
                }
                this._swuPgnDeckOrderCaptured.add(player.id);
                const order = this.captureDeckOrderFor(player);
                const seat = this.swuPgnSeat(player.id);
                if (!this._swuPgnInitialDeckOrder) {
                    this._swuPgnInitialDeckOrder = { p1: [], p2: [] };
                }
                if (seat === 1) {
                    this._swuPgnInitialDeckOrder.p1 = order;
                } else {
                    this._swuPgnInitialDeckOrder.p2 = order;
                }
                // Emit INIT once both seats have been captured (recordInit reads deckOrder()).
                const players = this.getPlayers();
                if (players.length >= 2 && players.every((p) => this._swuPgnDeckOrderCaptured.has(p.id))) {
                    this._swuPgnRecorder.recordInit();
                }
            } catch {
    ...
    private swuPgnDeckOrder(): { p1: string[]; p2: string[] } {
        return this._swuPgnInitialDeckOrder ?? { p1: [], p2: [] };
    }
    ...
    private buildSwuPgnReducedState(round: number): ReducedState {
        ...
        const state: ReducedState = {
            round,
            phase: phaseVocab(this.currentPhase as unknown as string),
            initiative: null,
            players: {},
        };

        try {
            const initPlayer = this.initiativePlayer;
            if (initPlayer) {
    ...
    private swuPgnResult(player1: Player): 'P1' | 'P2' | 'Draw' | 'Incomplete' {
        ...
        if (this.winnerNames.length > 1) {
            return 'Draw';
        }
        return this.winnerNames[0] === player1.name ? 'P1' : 'P2';
    }
    ...
    private generateSwuPgnFile(): string {
        return new SwuPgnWriter().write({
            header: buildHeader(this.swuPgnHeaderContext()),
            decks: this.buildSwuPgnDecks(),
            setup: this._swuPgnRecorder.getSetup(),
            events: this._swuPgnRecorder.getEvents(),
            annotations: [],
        });
    }
