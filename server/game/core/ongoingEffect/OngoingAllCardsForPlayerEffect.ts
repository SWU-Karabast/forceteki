...
export class OngoingAllCardsForPlayerEffect extends OngoingEffect<Card> {
    public readonly cardTargetMode: CardTargetMode;
    ...
    public override isValidTarget(target: Card) {
        if (this.cardTargetMode === CardTargetMode.OnlyOwned && target.owner !== this.player) {
            return false;
        }

        if (
            this.cardTargetMode === CardTargetMode.OwnedOrControlled &&
    ...
    public override getTargets() {
        switch (this.cardTargetMode) {
            case CardTargetMode.OnlyOwned:
                return this.game.allCards.filter((card) => card.owner === this.player);
            case CardTargetMode.OwnedOrControlled:
