describe('Stolen Starpath Unit', function () {
    integration(function (contextRef) {
        ...
        it('Stolen Starpath Unit\'s on attack ability should name a card, look at opponent\'s hand and create a spy token for each card which have the same name (no card with that name)', async function () {
            await contextRef.setupTestAsync({
                phase: 'action',
                player1: {
                    groundArena: [{ card: 'battlefield-marine', upgrades: ['stolen-starpath-unit'] }],
