    ...
    integration(function(contextRef) {
        ...
        describe('Kazuda\'s deployed unit on-attack ability', function() {
            ...
            it('can select himself to remove his own abilities for the round', function() {
                ...
                expect(context.player1).toHaveExactPromptButtons([
                    'Remove all abilities from any number of friendly units',
                    'Restore 2'
                ]);

                // Resolve his ability first
                context.player1.clickPrompt('Remove all abilities from any number of friendly units');
                expect(context.player1).toHavePrompt('Choose friendly units to lose all abilities for this round');
        ...
        describe('Kazuda Xiono\'s ability vs already-resolved triggered abilities', function() {
            // Ruling 2025: removing a unit's abilities does not retroactively nullify effects that were
            // already generated by a triggered ability that resolved earlier (e.g. a Shield token or
            // damage already created stays).
            xit('does not retroactively undo effects generated by an already-resolved triggered ability', function () {
                // A friendly unit's triggered ability resolves and generates a lasting effect/token
                // (e.g. gives a Shield). Later, Kazuda's ability removes that unit's abilities. The
