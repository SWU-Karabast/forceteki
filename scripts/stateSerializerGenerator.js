...
// refMap/refSet need more than a single assignment: assign an empty collection through the public
// accessor, then populate it via the live object's own overridden set()/add().
//
// The surviving reason for that shape is per-entry ref *marking*. GameObjectUtils.ts's UndoMap/UndoSet
// (backing @stateRefMap/@stateRefSet) override set()/add() to call markStateRef on each stored referent,
// which is what latches `_hasRef` and keeps the referent alive through the next
// `removeUnusedGameObjects()`. Handing the decoded collection to the accessor in one assignment would
// still mark (the accessor marks too), but populating through the override is what keeps the restored
// field a real UndoMap/UndoSet with its entries routed through the tracked API.
//
// P3-PB2 note: this comment previously described a *pre-initialization* hazard - the override used to read
// a private field (the state-bag mirror) that is not installed while the Map/Set base constructor is still
// running, so constructing one pre-populated threw "Cannot read private member ...". The cutover deleted
// the mirror and that private read, so the hazard is closed; the emitted shape is unchanged because the
// marking reason above still holds.
...
function emitArtifact({ targets, generationHash, visitedFiles, excludedFragmentClassNames }) {
    ...
    const interfaceBlocks = targets.map((target) => {
        ...
        // P3-PB2: a `type` alias, deliberately not an `interface`. The rollback hooks
        // (`GameObjectBase.afterSetState`/`afterSetAllState`/`cleanupOnRemove`) take
        // `SerializedStateRecord` = `Record<string, unknown>`, and an override narrowed to one of these
        // per-class shapes only type-checks if the shape has an implicit index signature. TypeScript grants
        // that to object *type literals* and not to interfaces, so emitting `interface` here makes every
        // retyped override fail with TS2416. Declaration form is not a schema-surface-hash input, so this
        // does not move `GENERATED_SCHEMA_SURFACE_HASH`.
        return `export type ISerialized${target.name} = {\n${members}\n};`;
    });
    ...
    return [
        ...
        '// change. This artifact is gitignored (server/game/core/generated/). Since P3-PB2 it is the sole',
        '// snapshot authority: GameStateManager and SnapshotFactory reach these serializers on every capture',
        '// and every rollback, so a defect here breaks live games, not just a parity check.',
        '',
