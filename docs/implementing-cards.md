...
Copy-paste these values into the card impl file, The final result should look like below:
...
If a card is missing from `test/json/_cardMap.json` after running `npm run get-cards`, it likely indicates that FFG has not yet added the card to their official database. To resolve this, you must manually define the card in `scripts/mockdata.js` and run `npm run get-cards` again.
