...
## `P3-PA2` — Codegen serializer parity harness, serialize leg (Plan 3, Phase A step 3)
...
| Date | 2026-09-16 |
| Lane / tier | full, tier 2 (Medium 🟡) |
| Plan | [03-codegen-serializers.md](03-codegen-serializers.md), Phase A step 3 |
| Parent | `2c6381f7a` |
| Commit | `02664636b` |
| Branch | `experimental/rollback-saves-optimizations` |

### Headline result: the generator passes the parity gate

This is the gate `experimental/ts-morph-v2` skipped — it deleted the old system in the same commit that introduced the new one, and never ran a comparison. The gate has now run at full engine scale:

| Run | Specs | Failures | Snapshots compared | Records compared | Comparison mismatches |
|---|---|---|---|---|---|
| `test-parity` | 8596 | 4 | 1,790 | 235,268 | **0** |
| `test-parity-undo` | 8408 | 80 | **78,602** | **10,895,600** | **0** |

Across ~10.9M serialized records, the generated serializers agree with the existing `getStateUnsafe()`+`v8` path exactly — every ref, ref array, tagged `Map`/`Set`, primitive and nested `stateValue` payload, in iteration order, per uuid. **Zero of the 84 failures are comparison mismatches.** Every one is an encoder *throw* on a single pre-existing data defect (see below). That is the substantive de-risking input for `P3-PB2`: the cutover's central unknown — whether generated serialization reproduces bag serialization — is now empirical rather than assumed, with exactly one known exception to clear first.

### What changed

Three files, all additive, all test-mode-only. No production file under `server/` is touched; the state bag, `copyState` and `v8.serialize` remain the sole snapshot authority.

- **`test/helpers/ParityHarness.ts`** (new). Auto-loaded by `jasmine.json`'s existing `helpers/**/*.js` glob, so no config edit was needed. At module scope, gated entirely behind `process.env.ENABLE_PARITY_HARNESS === 'true'`, it monkeypatches `GameStateManager.prototype.buildGameStateForSnapshot` — verified by grep to be the sole producer of per-object snapshot records, with one caller (`SnapshotFactory.createSnapshotForCurrentTimepoint`). The wrapper calls the original first, returns its exact `Buffer` untouched, and compares in between. Exports `getParityHarnessStats()`, a pure `compareSnapshotRecords(manager, buffer)` that does no prototype patching, and idempotent `installParityHarness()` / `uninstallParityHarness()`.
- **`test/server/core/ParityHarness.spec.ts`** (new, 10 cases, standard proof level) — see the test inventory below.
- **`package.json`** — four scripts: `jasmine-parity`, `jasmine-parity-undo`, `test-parity`, `test-parity-undo`.

### The comparator's three pinned semantics

All three were verified against the shipped code by two independent cold reviewers, not merely asserted:

1. **Side-effect-free.** Reads refs via `.uuid` only; `getObjectId()` is never referenced outside comments. This is what keeps `main`'s remove-then-serialize snapshot order valid — `getObjectId()` latches `_hasRef`, and a harness that latched during comparison would corrupt the very culling contract it exists to validate. `GameStateManager.get(uuid)` was confirmed to be a pure map lookup plus assert.
2. **The `undefined`(old)/`null`(new) exception is confined to top-level record keys, in that one direction only.** Every recursive call site — array element, plain-object key, `Map` value, `Set` member — passes `isTopLevel: false` explicitly; only the single top-level invocation passes `true`. The reverse direction (old `null`, new `undefined`) and the same pattern at any depth both fail loudly. Scoping this narrowly matters: `encodeStateValue` passes `undefined` and `null` through verbatim inside a `value` payload, so the pattern at depth > 0 can only mean a real encoder divergence, and a blanket nullish rule would have silently forgiven it.
3. **`Map`/`Set` compared in iteration order, never sorted.** Both sides walked index-aligned from `[...iterable]`; no `sort(` call exists in the file. Dispatch is on the format's own `$map`/`$set` tags from `STATE_ENCODING_TAGS`, reusing the same discriminant `decodeStateValue` relies on rather than inventing a parallel classification — and an unhandled reserved tag (`$num`, or any future addition) throws rather than falling through to lenient plain-object recursion.

A narrower fact than the plan doc's prose, established during planning and worth keeping: the `undefined`/`null` asymmetry is **exactly and only** the scalar `stateRef` case (`newValue?.getObjectId()` versus `encodeRef`'s `?? null`). Every container ref decorator already normalizes absent to `null` on both sides, and primitive/value fields read the same live storage on both sides in Phase A. That last point is Phase-A-scoped — see the notes for the next agent.

### The bug the harness found on its first full run

`DamageDealtThisPhaseWatcher.entries[n].damageSourceCardTypes[m]` holds `undefined` at runtime, in a field typed `CardType[]` with non-optional elements (`DamageDealtThisPhaseWatcher.ts:129-175`, the `setupWatcher` updater, which builds the array three different ways depending on `event.type`). `encodeStateValue` deliberately refuses an `undefined` array element because `JSON.stringify` would silently turn it into `null`; `v8.serialize` tolerated it silently, so this has been latent. The reachable production path is `SnapshotManager.moveToNextTimepoint` → `SnapshotFactory.createSnapshotForCurrentTimepoint` → `buildGameStateForSnapshot` → the narrowing.

Distribution in the undo run: 72 failures at entry index 0, 3 at index 1, 3 at index 2, 2 at index 3 — one root cause throughout.

Classified **stop-and-report** under the plan's own bounded dispositions, using both of its mechanical diagnostics: `getStateSerializerFor(instance).className` equals `instance.constructor.name` (so not an ancestor-resolution case), and the thrown message matches one of the four named `encodeStateValue` narrowing sites. Nothing was patched around it — no encoder change, no `STATE_RECORD_FORMAT_VERSION` bump, no schema-surface-hash regeneration, no comparator loosening. The fix is spun out to its own unit. `CardType.Leader` is well-defined and `buildTypeFromPrinted` throws on anything unexpected, so "the card has no type" is *not* the explanation; the cause is subtler and needs instrumenting rather than guessing.
...
|---|---|
| `npm run lint` | exit 0 |
| Test build (`scripts/build-test.js`, both trees) | exit 0, from a clean `build/` |
| New spec file alone, flag on / off / `--parallel=4` | 10 specs, 0 failures in every mode; flag-on reporter `snapshotsCompared=14 recordsCompared=1522` |
| `npm run test-parallel` (flag **off**) | 8596 specs, 0 failures, 13 pending — baseline 8586/0/13 plus exactly the 10 new specs |
| `npm run test-parallel-undo` (flag **off**) | 8408 specs, 0 failures, 17 pending — baseline 8398/0/17 plus the 10; none degraded to a pending stub |
| `npm run test-parity` | 4 failures, one root cause, 0 comparison mismatches (accepted residual) |
| `npm run test-parity-undo` | 80 failures, same root cause, 0 comparison mismatches (accepted residual) |
| Harness overhead (evidence only, not a `npm run benchmark` capture) | one heavy spec file: 2.747s flag off → 3.641s flag on, ≈1.33×, well under the raised 20000ms per-spec budget |

New tests, all in `test/server/core/ParityHarness.spec.ts` (10 cases, standard proof level): AC1 install-and-fired proof (flag-conditional, see below); AC2 side-effect-freedom under a `getObjectId`-throwing stub scoped to a direct `compareSnapshotRecords` call; AC3 exception scoping (4 cases — real unset scalar ref, synthetic top-level `undefined`-versus-real, the never-forgiven reverse direction, and a nested `value`-payload case proving the exception does not leak below depth 0); AC4 `Map` iteration order (2 cases — real `StateWatcherRegistrar.watchers` clean round trip, plus a reversed-order-on-one-side mismatch proving no sort); AC5 injected-mismatch pilot; AC9 unhandled reserved `$num` tag. No existing test was modified or removed. Every monkeypatch is `try`/`finally`-restored so none can leak between specs in a worker.

### Two deviations from the approved plan, both verified before acceptance

1. **The evidence reporter uses `process.on('exit', …)`, not `jasmine.getEnv().addReporter(…)`** as the plan specified. `jasmine-core` throws `"Reporters cannot be added via Env in parallel mode"` once `--parallel` is active, and both parity scripts use `--parallel=4`; `beforeAll`/`afterAll` from a helper file are equally restricted under parallel mode. The substitute is plain Node, fires once per worker process, and does not touch exit codes. It also resolves a defect the plan carried: jasmine's `parallel_worker.runSpecFile()` calls `env.execute()` once per spec **file** per worker, and `parallelReset()` keeps registered reporters, so a `jasmineDone` reporter would have emitted many cumulative lines per worker rather than one total.
2. **The AC1 spec calls `integration(fn, true)` through a cast rather than using `undoIntegration`.** Under `ENABLE_UNDO_ALL_TESTS=true`, `undoIntegration` is reassigned to a no-op `xit` stub (`IntegrationHelper.js:373`), which would have left the canonical harness-fired proof *pending* in exactly the run that needs it passing. `global.integration` genuinely accepts `(definitions, enableUndo)` at runtime; the cast through `unknown` is needed only because `IntegrationHelper.d.ts:16,18` under-declares both as single-argument.

### Review history

**Two plan reviews, each returning REJECTED with a blocking finding in the *verification* design rather than the harness design** — worth recording, because both would have produced a gate that passed while proving nothing:

- Round 1: (a) the planned scripts put `cross-env ENABLE_PARITY_HARNESS=true` on the **build** command followed by `&& npm run jasmine-*`, which does not deliver the variable to the jasmine process — this repo sets `ENABLE_UNDO_ALL_TESTS` *inside* `jasmine-undo-parallel` for exactly that reason. Both gate runs would have compiled the harness, never installed it, and exited 0 green. (b) No criterion produced positive evidence the harness fired: the uuid-count check compared the harness's set against the buffer it was derived from, so its falsifier could not fail, and the suite criterion observed exit code only. Plus 5 warnings and 5 nits, including the too-broad recursion-depth exception (2 above) and an unguarded "fix the generator" mandate with no carve-out for the encoder's deliberate narrowings.
- Round 2 (authorized budget extension beyond the tier-2 ceiling of 1): confirmed round 1's fixes genuine, then found that the plan body never carried the manifest's pinned flag-off branch for the AC1 spec — as committed coverage it also runs in the flag-off suites, where an unconditional `expect(installed).toBe(true)` would have failed every run and broken CI. Also corrected the reporter-granularity error described above. The four round-2 findings each arrived with a fully specified resolution and were transcribed by the orchestrator rather than costing a third planner round.

Implementation review round 1 — APPROVED WITH CONCERNS, 0 blocking, 1 warning (H1), 4 nits. Fix-cycle-1 disposition: **H1 fixed** (the primitive fallback used `!==`, which reports two identical `NaN` values as a divergence; `@statePrimitive` has no JSON-safety assert so `NaN` is representable today — now `!Object.is(...)`), **H2/H3/H4 fixed** (`describeForError` always returns a string; `$map`/`$set` payloads validated as arrays before destructuring so a malformed tag yields an attributed message rather than a raw `TypeError`; `entry.className` threaded through every call site so the plan's ancestor-resolution triage test is derivable straight from the failure text), **H5 rejected** as out of file scope. Round 2 delta review — **APPROVED**, 0 blocking, 0 warning, 1 informational nit; it independently re-ran the spec file and reproduced the reported numbers exactly, and re-confirmed all three pinned invariants intact after the repairs.

`Object.is` is not a strict relaxation of `!==` — it treats `-0` and `+0` as distinct — so both parity suites were re-run after the repair specifically to rule out a new signed-zero divergence empirically rather than by argument. The post-fix failure set is identical to the pre-fix set, entry-index distribution included. The delta reviewer additionally traced it structurally and found signed zero unreachable in Phase A, since neither leg performs arithmetic on a primitive field and `v8.serialize` preserves the sign bit.

### Known residuals, disclosed and accepted at the gate

1. **AC7 is not satisfied and is recorded as accepted-risk, not passed.** Both harness-active suites fail on the `DamageDealtThisPhaseWatcher` finding above. Readiness reports `all_passed: false` with `resolved: true`; the failing outcome stays visible in the evidence log rather than being promoted. CI is unaffected — it runs lint, `validate-cards`, `test-parallel` and `test-parallel-undo`, never the parity scripts — and the flag-off suites are green, which is why this could land without a red pipeline.
2. **`npm run test-parity` (undo off) reaches far less of the engine than its name suggests.** `SnapshotManager` early-returns under `UndoMode.Disabled`, and only 10 of 2070 spec files call `undoIntegration`, so the undo-on run is where the real exercise volume lives (78,602 snapshots versus 1,790). Both are kept as required checks with per-run counts recorded so the asymmetry is explicit rather than implied.
3. **The per-run count is a derived figure, not a single printed total.** The reporter emits one cumulative line per worker process, tagged with `pid=`. The binding non-vacuousness check is "at least one line reports a nonzero `snapshotsCompared`"; the reported figure is the sum over distinct pids of each pid's maximum.
4. **No committed spec exercises the literal-`NaN` equality path** through the comparator's primitive fallback (delta review J1, raised and explicitly not actioned at standard proof level within the approved 10-case cap). That branch is covered by code trace, not by a runnable falsifier in this file.
5. **`installParityHarness()` raises `jasmine.DEFAULT_TIMEOUT_INTERVAL` to 20000ms process-globally** while installed, affecting every spec in that worker, not just parity-exercised ones. The trade-off is deliberate and one-directional in effect: a genuinely hung spec is reported later (20s rather than 5s), not masked. `uninstallParityHarness()` restores the prior value.
6. **`IntegrationHelper.d.ts` still under-declares `integration`/`undoIntegration` as single-argument** (implementation review H5, rejected as out of this unit's three task-owned files). Any future spec needing `enableUndo` will need the same cast until someone widens the ambient declaration.
7. **Process residual, recorded so the evidence provenance is not overstated.** The fix-pass agent authored all four repairs but stopped repeatedly on background waits without collecting them and never delivered a status block; its invocation is recorded as `interrupted`, and the orchestrator executed and recorded the remaining verification itself. During that overlap the agent and the orchestrator each started `test-parity` concurrently, both running `build-test.js` into the same `build/` tree and writing the same log filename. Both were stopped, the contaminated logs were **deleted unread**, `build/` was removed, and both parity suites were re-run serially from a clean rebuild — the remediation `CLAUDE.md` itself prescribes for contaminated build state. No number from the contaminated window is used as evidence anywhere. The earlier lint, unit-spec and flag-off runs completed before the overlap and are retained.

### Notes for the next agent

- **`P3-PA3` (restore leg) will almost certainly hit the `DamageDealtThisPhaseWatcher` bug too**, since it exercises the same watcher state. That fix is not optional for the rest of Phase A, merely separable. Check whether it has landed before planning, and consume its answer rather than rediscovering it.
- **The "primitive and value fields read the same live storage on both sides" invariant is Phase-A-scoped.** It is what makes signed zero unreachable today and what makes primitive parity true by construction. Phase B step 1 slims the decorators and breaks it; whichever unit does that slimming should add a signed-zero regression spec and should not assume primitive fields remain trivially in agreement (delta review Q1).
- **The harness is reusable and is meant to be**, per the design doc's own step 8. `compareSnapshotRecords(manager, buffer)` is exported and pure — no prototype patching — so a spec can call it directly with a stub scoped to just that call. That is how the side-effect-freedom case is proven without a `getObjectId`-throwing stub spanning gameplay, which would fail inside production ref setters rather than in the harness.
- **When a real mismatch fires, the failure text now carries both class names** (`class=X serializerClass=Y` when they differ), so the plan's ancestor-resolution triage test can be run straight off the message without re-deriving anything.
