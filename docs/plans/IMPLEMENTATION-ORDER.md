...
- Says (`:85-90`) `SnapshotManager.ts:457-474` for `opponentActedSinceLastSnapshot`; now `:457-475` (coincidentally stable). The `TODO THIS PR` at `:328` is still at `:328`.
...
Nine units: seven unconditional and two (`P4-E`, `P4-F`) that run only if `P4-D`'s verdict is "full deltas". The decomposition follows the plan's own instruction to land hooks, then memoization, then deltas "so the fallback is the intermediate landed state rather than a rewrite".
...
**The four open issues inherited from `-morph`, enumerated:**

| Issue (`04:325-342`) | Resolvable from current code? | Disposition |
|---|---|---|
| Phase-boundary prompts — `SnapshotManager.ts:328` `TODO THIS PR` (Sneak Attack, Thrawn) | No; it is a behavior gap in `getQuickRollbackPoint` that exists independently of deltas and needs a decision plus specs | Own unit, `P4-0`, run first as the standalone precursor the plan itself suggests |
...
| Unit | Scope | Depends on | Size | Route |
|---|---|---|---|---|
| `P4-0` | Inherited open issue — phase-boundary prompt quick-rollback policy (`SnapshotManager.ts:328`) | — | Medium 🟡 | full |
...
| `P4-F` *(conditional)* | The delta cutover: rollback protocol, hollow current snapshot + window-start materialization, always-full manual snapshots, harness delta-parity mode | `P4-E`, `P4-0`, `P4-1` | Large 🔴 | `--tier 4` |
| `P4-G` ⏱ | Cross-cutting spec sweep, delta diagnostic benchmark rows (new rows only), `after-plan-04` capture, the roadmap performance verdict | `P4-D` (memo branch) or `P4-F` (delta branch) | Medium 🟡 | full |

`P4-0` and `P4-1` are independent of everything and may run first or alongside `P4-A`. The chain: `P4-A → P4-B → P4-C → P4-D → [P4-E → P4-F] → P4-G`.
...
```
P4-0 → P4-1 → P4-A → P4-B → P4-C → P4-D ⏱(interim) → [P4-E → P4-F] → P4-G ⏱
...
- **`P4-0`, `P4-1`, `P6-G` at any time**, including before Plan 4 starts. `P4-0` is a quick-rollback-policy fix on main with no delta code; `P4-1` is a harness change; `P6-G` is engine-side `until` syntax plus three card files, depending on nothing in Plans 4 or 5. None of the three touches `GameStateManager`.
